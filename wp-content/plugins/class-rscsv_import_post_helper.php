...
class RSCSV_Import_Post_Helper
{
     ...
     * @return (RSCSV_Import_Post_Helper)
     */
    public static function add($data, $thumbnail = '')//Añadida posibilidad de insertar thumnail image
    {
        $object = new RSCSV_Import_Post_Helper();
        ...
        if ($data['post_type'] == 'attachment') {
			...
			echo "$post_id : ".$data['post_name']."<br>";
			if(!is_wp_error( $post_id ) && !empty($thumbnail)){//Si es el id de la thumbnail image
				if(is_numeric($thumbnail)){
					update_post_meta($post_id, '_thumbnail_id', $thumbnail);
				}
				else{//Si nos viene la url de la image
					$thumbnail_id = url_to_postid($thumbnail);
    ...
    public function update($data)
    {
        ...
        if (is_wp_error($post_id)) {
            ...
            echo "No es necesario copiar el id, $post_id : ".$data['post_name']." actualizado con éxtio.<br>";
